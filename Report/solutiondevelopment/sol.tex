...
\subsubsection*{Choice of $\Vec{A_n}$}
...
The problem reduces to finding the choice of $t_k(\Vec{x})$ that minimizes $J$. $J$ is the objective function for a k-means classifier where the class means are known. Just as in the case of a k-means classifier we choose $t_k(\Vec{x})$ to be defined as follows:

...
def writeGeo(name, firstIter=False ,**kwargs):
    ...
    if firstIter == False:
        subst = ""
        loopMesh = re.sub(r"Mesh\.MeshSizeMax.*=.*;", subst, loopMesh, 0, re.MULTILINE)    
        loopMesh = re.sub(r"Mesh\.MeshSizeMin.*=.*;", subst, loopMesh, 0, re.MULTILINE)
        
        fieldMesh = re.sub(r"Mesh\.MeshSizeMax.*=.*;", subst, fieldMesh, 0, re.MULTILINE)
        fieldMesh =     re.sub(r"Mesh\.MeshSizeMin.*=.*;", subst, fieldMesh, 0, re.MULTILINE)
...
if __name__ == "__main__":
    ...
    for _R,_W in tqdm(zip(Rs,Ws)):
        ...
        for k in range(4):
            ...
            """ remesh """
            mesh_start = time()
            n = remesh("./output/J_washer.vtk",names,0.5)
            mesh_stop = time()
